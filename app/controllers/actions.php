...
class Actions extends ControllerApp{
	...
	function index()
	{
		...
		// if($this->Members->isAdmin()){
			// $sql_pub = "1";
		// }
		$this->f3->set('actions',$this->Actions->paginate(1,1000,'`when`','ASC',"WHERE tipo = 'antifrack' AND `when` >= (NOW() - interval 1 day) OR until >= NOW() ",'id'));
		
		// $this->f3->set('meta.title',"Agenda anti-fracking e não convencionais - ".$this->f3->get('SITE_TITLE'));
		...
		$this->f3->set('page',$this->Configs->getLink('/actions/'));
	}
